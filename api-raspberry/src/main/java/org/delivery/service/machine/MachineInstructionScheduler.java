...
public class MachineInstructionScheduler {
    ...
    public MachineInstruction getMachineMoveInstruction(final MachineTargetRequest machineTargetRequest) {
        final MachineCoordinate x = machineInfo.getMachineCoordinateX(machineTargetRequest.getXRate(), machineTargetRequest.getYRate());
        final MachineCoordinate y = machineInfo.getMachineCoordinateY(machineTargetRequest.getXRate(), machineTargetRequest.getYRate());

        return new MachineMoveInstruction(machineConnection, new MachineCode(Gcode.G00, x, y, MachineCoordinate.nothing()));
    }
    ...
    public MachineInstruction getReturnToZeroInstruction() {
        return new MachineMoveInstruction(machineConnection, new MachineCode(Gcode.G00, machineInfo.getZeroCoordinateX(), machineInfo.getZeroCoordinateY(), MachineCoordinate.nothing()));
    }
